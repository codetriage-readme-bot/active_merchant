    ...
    class MundipaggGateway < Gateway
      ...
      def commit(action, parameters, auth = nil)
        ...
        Response.new(
          ...
          when '400'
            return Response.new(false, "Invalid request; #{message}", {}, :test => test?)
          when '401'
            return Response.new(false, "Invalid API key; #{message}", {}, :test => test?)
          when '404'
            return Response.new(false, "The requested resource does not exist; #{message}", {}, :test => test?)
          when '412'
            return Response.new(false, "Valid parameters but request failed; #{message}", {}, :test => test?)
          when '422'
            return Response.new(false, "Invalid parameters; #{message}", {}, :test => test?)
          when '500'
            return Response.new(false, "An internal error occurred; #{message}", {}, :test => test?)
          end
          raise
      ...
      def success_from(response)
        %w[pending paid processing canceled].include? response['status']
      end
