...
class MundipaggTest < Test::Unit::TestCase
  ...
  def test_successful_refund
    ...
    assert_equal 'O5L5A4VCRK', response.authorization
    assert response.test?
  end
  ...
  def test_successful_void
    ...
    response = @gateway.void('K1J5B1YFLE')
    assert_success response
  ...
  def test_successful_verify
    ...
    response = @gateway.verify(@amount, @credit_card, @options)
    assert_success response

    assert_equal 'K1J5B1YFLE', response.authorization
    assert response.test?
  end
